# task_control —— 任务控制线的 ROS2 缝隙（Jetson 上跑）
...
| TaskController 缝隙 | 离线(examples) | ROS2(本目录) |
|---|---|---|
| `scan()` → 每轮识别 det 列表 | `OfflineTable.scan`（网格中心反算像素合成） | `ScanFromDetections.latest`（订阅 Detection2DArray，乙/仿真 mock 节点发布） |
| `pick_place(cell_id, cls)` → ('ok'|reason, note) | `OfflineTable.pick_place`（移走桌面物体） | `PickPlaceActionClient.pick_place`（发 PickPlace Action） |

- Action server 骨架：`pick_place_server.py`（时序 + A/B 解算 + reason 收尾，机器人无关）
- 检测订阅：`scan_from_detections.py`

> 本目录文件需要 ROS2 + `sort_msgs` 编译产物 + vision_msgs，**只能在 Jetson 上跑**（Mac 离线只做语法检查）。
> 跑法继承 c4_2 风格：直接 `python3 <file>`，不是 colcon 包。

## 三件套怎么拼（Jetson，main 线程编排同 c4_2/drive/c4_cycle.py）
...
### 真执行者已经写好：`c4_executor.C4PickExecutor`

它把上面 6 段动作**实现完了**（组合 C4Driver 当 robot、只调运动原语；A/B 由 server 按 goal 解算），
Jetson 上把真 C4Driver 实例传进去即可，不用手写 6 段：

```python
# 在 Jetson 上、C4Driver 实例就绪后（构造方式见 c4_2/drive/c4_cycle.py main()）
...
多 A/B 由 server 解算、判 held/placed 默认 assume。真判据在 Jetson 联调时对 `C4PickExecutor`
override `_judge_hold(ctx)` / `_judge_place(ctx)`（读物体位姿、或人工确认，对齐 c4_2 judge=manual）。
`motion` 数值联调时按甲标定覆盖 —— 这部分只能在 Jetson/Gazebo 上验证，Mac 只做了时序/失败传播的
...
- bbox 是归一化 `[0,1]` 原点左上（乙骨架同款），本目录按 config 图像尺寸还原像素；若口径改像素直出，
  改 `scan_from_detections.dets_from_msg` 里 `_norm_to_px`（或加参数关掉）。
