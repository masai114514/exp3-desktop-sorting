...
\begin{itemize}[leftmargin=1.6em]
  ...
  \item \textbf{不能声称}：HSV 检测算法在整轮任务中稳定有效——渲染停帧之后拿不到全程证据。
        渲染健康的第 1 轮 HSV 实测 \textbf{6/6 全部正确映射}，这份证据单独保留。
\end{itemize}
...
\textit{定位：}用 HSV 检测器跑到第 3$\sim$4 轮，扫描日志里的检测数从 6 掉到 1，
且连续三轮\textbf{完全相同}；帧戳照常推进（173.9$\to$181.4$\to$189.1\,s），
画面内容却一字不变。物理侧完全正常（物块坐标与臂位姿都是实时的），
所以死的是\textbf{渲染管线}，不是算法。根因是容器无 GPU：
\texttt{color\_detector} 吃 479\% CPU、\texttt{gzserver} 吃 1013\%，渲染抢不到资源。
